...
func TraceBlock(ctx context.Context,
	...
	for th := 0; th < threads; th++ {
		...
		_ = defaultPool.Submit(func() {
			...
			for task := range jobs {
				msg, _ := transactionToMessage(txs[task.index], signer, block.BaseFee())
...
func transactionToMessage(tx *types.Transaction, s types.Signer, baseFee *big.Int) (*types.Message, error) {
	var gasPrice *big.Int
	// If baseFee provided, set gasPrice to effectiveGasPrice.
	if baseFee != nil {
		gasPrice = cmath.BigMin(new(big.Int).Add(tx.GasTipCap(), baseFee), new(big.Int).Set(tx.GasFeeCap()))
	} else {
